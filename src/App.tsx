...
// --- LOGIC CALCULATE HEALTH ---
const calculateProjectHealth = (p: Project) => {
    const totalRAB = (p.rabItems || []).reduce((acc, item) => acc + (item.volume * item.unitPrice), 0);
    let realProgress = 0;
    if (totalRAB > 0) { (p.rabItems || []).forEach(item => { const w = ((item.volume * item.unitPrice) / totalRAB) * 100; realProgress += (item.progress * w) / 100; }); }
...
// --- HELPER FUNCTION: Get Grouped Transactions ---
...
const App = () => {
  ...
  const updateProject = async (data: Partial<Project>) => { 
    ...
    setIsSyncing(true); try { await updateDoc(doc(db, 'app_data', appId, 'projects', activeProjectId), data); } catch(e) { alert("Gagal simpan."); } setIsSyncing(false); 
  };
  ...
  const getRABGroups = () => { if(!activeProject) return {}; const g: any = {}; activeProject.rabItems?.forEach(i => { if(!g[i.category]) g[i.category] = []; g[i.category].push(i); }); return g; };
  ...
  const handleGenerateRAB = async () => {
    ...
    const apiKey = ""; // API Key disembunyikan sesuai instruksi
    try {
                            ...
                            <div className="lg:col-span-3">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <div className="flex gap-2">
                                        {/* AI BUTTON */}
                                        {canEditProject() && (
                                            <button onClick={() => { setModalType('aiRAB'); setShowModal(true); }} className="text-xs bg-purple-100 text-purple-700 px-3 py-2 rounded-lg font-bold border border-purple-200 hover:bg-purple-200 flex items-center gap-1">
                                                <Sparkles size={14}/> Auto RAB
                                            </button>
                                        )}
                                        {canEditProject() && <button onClick={handleAddCCO} className="text-xs bg-orange-100 text-orange-700 px-3 py-2 rounded-lg font-bold border border-orange-200 hover:bg-orange-200">+ CCO</button>}
                                        {canEditProject() && <button onClick={() => { setSelectedRabItem(null); setModalType('newRAB'); setShowModal(true); }} className="text-xs bg-blue-600 text-white px-3 py-2 rounded-lg font-bold hover:bg-blue-700 shadow">+ Item Baru</button>}
                                    </div>
                                </div>
               ...
               {modalType === 'aiRAB' && (
                 <>
                   <div className="bg-purple-50 p-3 rounded-lg border border-purple-200 mb-2">
                      <h4 className="text-purple-700 font-bold text-sm flex items-center gap-1"><Sparkles size={14}/> AI Generator</h4>
                      <p className="text-xs text-purple-600 mt-1">Jelaskan proyek Anda, AI akan membuatkan RAB lengkap.</p>
                   </div>
                   <textarea className="w-full p-3 border rounded-lg text-sm h-32" placeholder="Contoh: Bangun pos satpam 3x3m, dinding bata merah, atap baja ringan, lantai keramik, ada 1 titik lampu dan saklar." value={aiPrompt} onChange={e => setAiPrompt(e.target.value)}></textarea>
                   <button onClick={handleGenerateRAB} disabled={isGeneratingAI} className="w-full bg-purple-600 text-white p-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-purple-700 disabled:opacity-50">
                     {isGeneratingAI ? <Loader2 className="animate-spin"/> : <Sparkles size={18}/>} {isGeneratingAI ? 'Sedang Membuat...' : 'Generate RAB Otomatis'}
                   </button>
                 </>
               )}
