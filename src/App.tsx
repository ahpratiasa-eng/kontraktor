...
// Pastikan file-file ini sudah dibuat di iPad Anda sesuai struktur folder
                    ...
                    <main className="p-8 max-w-4xl mx-auto">
                        ...
                        {rabViewMode === 'client' ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div><h4 className="font-bold text-green-700 border-b mb-2">Pemasukan</h4>{getGroupedTransactions(activeProject.transactions.filter(t=>t.type==='income')).map(g=><div key={g.id} className="text-xs flex justify-between border-b py-1"><span>{g.category}</span><span>{formatRupiah(g.totalAmount)}</span></div>)}</div>
                                <div><h4 className="font-bold text-red-700 border-b mb-2">Pengeluaran</h4>{getGroupedTransactions(activeProject.transactions.filter(t=>t.type==='expense')).map(g=><div key={g.id} className="text-xs flex justify-between border-b py-1"><span>{g.category}</span><span>{formatRupiah(g.totalAmount)}</span></div>)}</div>
                            </div>
                            </>
                        )}
                    </main>
